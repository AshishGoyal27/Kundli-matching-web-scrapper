...
def getQueryResult(b_name, b_dob, b_tob, b_pob, g_name, g_dob, g_tob, g_pob):
    ...
    driver.get(url)
    sleep(6)
    element = driver.find_element_by_xpath('//input[@id="name1"]')
    element.send_keys(b_name)
    m = b_dob.split("-")
    element1 = driver.find_element_by_xpath('//select[@name="day1"]')
    if m[0][0] == "0":
        element1.send_keys(m[0][1])
    else:
        element1.send_keys(m[0])
    
    element2 = driver.find_element_by_xpath('//select[@name="month1"]')
    element2.send_keys(m[1])
    element3 = driver.find_element_by_xpath('//input[@name="year1"]')
    element3.send_keys(m[2])
    n = b_tob.split(":")
    element4 = driver.find_element_by_xpath('//select[@name="hrs1"]')
    if n[0][0] == "0":
        element4.send_keys(n[0][1])
    else:
        element4.send_keys(n[0])

    element5 = driver.find_element_by_xpath('//select[@name="min1"]')
    if n[1][0] == "0":
        element5.send_keys(n[1][1])
    else:
        element5.send_keys(n[1])

    element6 = driver.find_element_by_xpath('//select[@name="sec1"]')
    if n[2][0] == "0":
        element6.send_keys(n[2][1])
    else:
        element6.send_keys(n[2])
    
    element7 = driver.find_element_by_xpath('//input[@name="place1"]')
    element7.send_keys(b_pob)

    elementg1 = driver.find_element_by_xpath('//input[@id="name2"]')
    elementg1.send_keys(g_name)
    m1 = g_dob.split("-")
    elementa2 = driver.find_element_by_xpath('//select[@name="day2"]')
    if m1[0][0] == "0":
        elementa2.send_keys(m1[0][1])
    else:
        elementa2.send_keys(m1[0])
    element13 = driver.find_element_by_xpath('//select[@name="month2"]')
    element13.send_keys(m1[1])
    element14 = driver.find_element_by_xpath('//input[@name="year2"]')
    element14.send_keys(m1[2])
    element15 = driver.find_element_by_xpath('//select[@name="hrs2"]')
    n1 = g_tob.split(":")
    ...
    element16 = driver.find_element_by_xpath('//select[@name="min2"]')

    if n1[1][0] == "0":
        element16.send_keys(n1[1][1])
    else:
        element16.send_keys(n1[1])
    element17 = driver.find_element_by_xpath('//select[@name="sec2"]')
        
    if n1[2][0] == "0":
        element17.send_keys(n1[2][1])
    else:
        element17.send_keys(n1[2])
    element18 = driver.find_element_by_xpath('//input[@name="place2"]')
